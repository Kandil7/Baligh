...
class PromptFormatter:
    ...
    def format_cpt(self, example):
        ...
        tokenized = self.tokenizer(text, truncation=True, max_length=self.max_seq_length, padding=False, return_tensors=None)
    ...
    def format_sft(self, example):
        ...
        formatted = apply_chat_template(self.tokenizer, messages, tokenize=True, add_generation_prompt=False)
        if isinstance(formatted, str):
            tokenized = self.tokenizer(formatted, truncation=True, max_length=self.max_seq_length, padding=False, return_tensors=None)
        else:
    ...
    def format_chat(self, messages):
        return apply_chat_template(self.tokenizer, messages, tokenize=True, add_generation_prompt=True)
