...
from baligh.data import (
    ...
    get_cpt_formatter, get_sft_formatter,
    validate_dataset, check_dataset_schema
)
...
Examples:
  ...
  python -m src.scripts.prepare_data --stage cpt --clean --max-samples 1000
    ...
    parser.add_argument(
        "--clean",
        action="store_true",
        help="Apply cleaning pipeline"
    )
    parser.add_argument(
        "--deduplicate",
        action="store_true",
        help="Apply near-duplicate removal (MinHash LSH)"
    )
    parser.add_argument(
        "--streaming",
        action="store_true",
    ...
    parser.add_argument(
        "--push-to-hf",
        action="store_true",
        help="Push prepared data to Hugging Face Hub"
    )
    parser.add_argument(
        "--hf-repo",
        type=str,
