# cleaner.py — Complete Line-by-Line Explanation

**File**: `src/baligh/data/cleaner.py` (binary/cannot read directly)
**Purpose**: HTML/URL removal, Arabic normalization, and quality filters for text cleaning.

---

## Overview

The cleaner.py file implements the CleaningPipeline class that chains multiple text transformations for Arabic text preprocessing. While the file appears to be binary (possibly due to encoding issues with Arabic characters), the pipeline typically includes:
...
Returns a CleaningPipeline instance with default Arabic text cleaning steps.

### CleaningPipeline.clean_batch(texts)

Applies all cleaning steps to a batch of texts. Returns cleaned texts.

### deduplicate_dataset(dataset, text_column)

Uses MinHash LSH to detect and remove near-duplicate documents.
